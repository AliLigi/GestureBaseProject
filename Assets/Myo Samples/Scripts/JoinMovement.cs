...
public class JoinMovement : MonoBehaviour {
...
// Making the fingers spread pose or pressing the 'r' key resets the reference orientation.
    ...
    void Update()
    {
        ...
        if (updateReference)
        {
            ...
            referenceVector = new Vector2(myo.transform.forward.x * 50, myo.transform.forward.y * 35);
        }
        transform.position = new Vector2((myo.transform.forward.x * 50) - referenceVector.x, myo.transform.forward.y * 35 - referenceVector.y);
        ...
        //transform.rotation = _antiYaw * antiRoll * Quaternion.LookRotation(myo.transform.up);
